...
public class CustomChatScreen extends AbstractContainerCursorScreen<ChatMenu> {
    ...
    protected void renderLabels(@NotNull GuiGraphics graphics, int i, int j) {
        graphics.drawString(this.font, "Chat", this.titleLabelX, this.titleLabelY, 4210752, false);
        int online = this.menu.usernames().size();
        graphics.drawString(this.font, online + "/" + this.menu.maxUsers() + " Online", 198, this.titleLabelY, 4210752, false);
        String charCount = ClientUtils.getSmallNumber(ChatMessage.MAX_MESSAGE_LENGTH - box.getValue().length());
        graphics.drawString(this.font, charCount, 191 - this.font.width(charCount), 137, 4210752, false);
    }
