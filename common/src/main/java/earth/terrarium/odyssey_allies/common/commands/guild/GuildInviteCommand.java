...
public final class GuildInviteCommand {
    ...
    private static void invite(CommandSourceStack source, ServerPlayer targetPlayer) throws CommandSyntaxException {
        ...
        targetPlayer.displayClientMessage(ModUtils.translatableWithStyle("command.odyssey_allies.guild_invited", player.getName(), guild.displayName()), false);
        targetPlayer.displayClientMessage(ConstantComponents.CLICK_TO_ACCEPT.copy().withStyle(Style.EMPTY
            .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, ModUtils.translatableWithStyle("command.odyssey_allies.join", guild.displayName())))
