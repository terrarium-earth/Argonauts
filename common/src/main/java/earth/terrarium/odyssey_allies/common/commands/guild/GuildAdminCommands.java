    ...
    private static void join(CommandSourceStack source, UUID id) throws CommandSyntaxException {
        ...
        if (Settings.ANNOUNCE_JOIN.get(guild)) {
            guild.onlineMembers(source.getLevel())
                ...
                .forEach(member -> member.displayClientMessage(ModUtils.translatableWithStyle("command.odyssey_allies.joined_guild", player.getName()), false));
        }
        source.sendSuccess(() -> ModUtils.translatableWithStyle("command.odyssey_allies.join_guild", guild.displayName()), false);
    }
