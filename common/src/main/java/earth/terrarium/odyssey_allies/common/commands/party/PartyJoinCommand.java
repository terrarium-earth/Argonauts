    ...
    private static void join(CommandSourceStack source, ServerPlayer targetPlayer) throws CommandSyntaxException {
        ...
        if (Settings.ANNOUNCE_JOIN.get(party)) {
            party.onlineMembers(source.getLevel())
                ...
                .forEach(member -> member.displayClientMessage(ModUtils.translatableWithStyle("command.odyssey_allies.joined_party", player.getName()), false));
        }
        source.sendSuccess(() -> ModUtils.translatableWithStyle("command.odyssey_allies.join_party", party.displayName()), false);
    }
}
