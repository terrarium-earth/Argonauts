...
public final class GuildAllyCommands {
    ...
    private static void add(CommandSourceStack source, ServerPlayer targetPlayer) throws CommandSyntaxException {
        ...
        source.sendSuccess(() -> ModUtils.translatableWithStyle("command.odyssey_allies.add_ally", targetPlayer.getName()), false);
        targetPlayer.displayClientMessage(ModUtils.translatableWithStyle("command.odyssey_allies.now_allied", guild.displayName()), false);
    }
    ...
    public static void remove(CommandSourceStack source, ServerPlayer targetPlayer) throws CommandSyntaxException {
        ...
        source.sendSuccess(() -> ModUtils.translatableWithStyle("command.odyssey_allies.remove_ally", targetPlayer.getName()), false);
        targetPlayer.displayClientMessage(ModUtils.translatableWithStyle("command.odyssey_allies.no_longer_allied", guild.displayName()), false);
    }
        ...
        guild.members().forEach((id, member) -> {
            if (member.status() == MemberStatus.ALLIED) {
                profileCache.get(id).ifPresent(profile -> source.sendSuccess(() ->
                    ModUtils.translatableWithStyle("command.odyssey_allies.list_ally", profile.getName()), false));
            }
        });
    }
}
