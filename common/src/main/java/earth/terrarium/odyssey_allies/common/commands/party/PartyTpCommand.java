...
public final class PartyTpCommand {
    ...
    private static void tp(CommandSourceStack source, ServerPlayer target) throws CommandSyntaxException {
        ...
        if (!party.hasPermission(player.getUUID(), Permissions.TELEPORT)) throw AlliesExcepetions.NO_PERMISSION_TELEPORT.create();
        if (!Settings.PASSIVE_TELEPORT.get(party)) throw AlliesExcepetions.PASSIVE_TELEPORT_DISABLED.create();
